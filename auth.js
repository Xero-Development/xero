document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Fast-path: session exists => dashboard
  try {
  ...
  loginPanel.addEventListener("submit", async (e) => {
    ...
    if (!data?.session) {
      loginStatus.textContent = "Signed in, but no session yet. If email confirmation is enabled, confirm your email and try again.";
      return;
    }
